...
<div class="content">
    ...
    <form class="form" action="/confirm" method="post">
        ...
        @if($errors->any())
            <div class="form__error">
                <ul>
                    ...
                    <li>{{ $error('last_name') }}</li>
                    <li>{{ $error('first_name') }}</li>
                    @endforeach
                </ul>
            </div>
            @endif
        ...
        <div class="form__group">
            ...
            <div class="form__input">
                <label for="man">
                    <input type="radio" class="form__input-radio" name="gender" id="man" value="1" value="{{ old('gender') == '1' ? 'checked' : '' }}">男性
                </label>

                <label for="woman">
                    <input type="radio" class="form__input-radio" name="gender" id="woman" value="2" value="{{ old('gender') == '2' ? 'checked' : '' }}">女性
                </label>

                <label for="other">
                    <input type="radio" class="form__input-radio" name="gender" id="other" value="3" value="{{ old('gender') == '3' ? 'checked' : '' }}">その他
                </label>
            </div>
        </div>
        ...
        <div class="form__group">
            ...
            <div class="form__select">
                <select name="category_id" class="form__select-item">
                    ...
                    <option value="{{ $category->id }}" value="{{ old('content') == $category->id ? 'selected' : '' }}">
                        {{ $category->content }}
                    </option>
                    @endforeach
                </select>
